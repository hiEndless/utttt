...
**采集内容**：
- **订单簿深度**：实时买卖盘数据
- **强平订单流**：实时强平订单（forceOrder）
- **价格波动检测**：检测价格异常波动（spike detection）

**技术实现**：
- **不使用 LLM**：WebSocket 实时数据流处理
- **强平统计**：实时累加强平订单数量和数量，写入 `force_stats:binance:BTCUSDT`
- **波动检测**：使用 Z-Score 和百分比变化检测异常波动
- **数据存储**：写入 Redis Stream `force_stream:binance:BTCUSDT`

**为后续提供**：
- 订单簿数据 → 市场结构分析（订单簿结构）
- 强平数据 → ForceStats Agent 分析
- 波动事件 → 事件中心生成市场事件
