...
| **市场 WebSocket** | `python -m data_server.binance.ws_binance.market_ws` | 实时订单簿、强平、价格，需先有 `symbol:binance` |
...
| **Agent 服务（一体）** | `python -m agent_server.main` | 同时跑 background + final_listen + risk_state_cron |
| **仅 Final 监听** | `python -m agent_server.final_listen_main` | 仅消费 `final_events`，跑信号验证与交易事件工作流 |
| **仅后台分析** | `python -m agent_server.background_main` | 仅 K 线 + 市场结构后台任务 |
| **仅风控定时** | `python -m agent_server.risk.risk_state_cron` | 仅每 60 秒更新全局风控叠加层 |
| **查询仓位** | `python -m agent_server.tools.get_position` | 默认查 binance ETHUSDT，可改代码或传参 |
...
**依赖关系简要**：`data_server` → `event_center`（依赖 Redis 指标）→ `api`（依赖 Redis 数据）→ `agent_server`（依赖 api HTTP + Redis final_events）。用户数据与交易事件需 `user_ws` 运行。
...
agent_server 包含主服务（background + final_listen + risk_state_cron），同时运行后台分析、最终事件监听和风控定时任务。
...
**注意**：
- agent_server 依赖 api 服务，它会通过 HTTP 请求调用 api 服务获取市场数据
...
```bash
# 查看所有 Python 进程
ps aux | grep python | grep -E "(rest_binance|event_center|api|agent_server)"
