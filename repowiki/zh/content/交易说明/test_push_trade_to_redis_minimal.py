...
DEFAULT_PRICE = float(os.environ.get('TEST_PRICE', '0.17774'))  # 当前市价（用于计算）
...
# 交易数量配置（二选一）
# 方式1：指定保证金（推荐）- 根据保证金、杠杆和价格动态计算数量
...
DEFAULT_TP_TRIGGER_PX = float(os.environ.get('TEST_TP_TRIGGER_PX', '60.0'))
DEFAULT_SL_TRIGGER_PX = float(os.environ.get('TEST_SL_TRIGGER_PX', '50.0'))
...
def create_trade_json(order_type: str,
    ...
    Args:
        ...
        margin: 保证金（USDT，可选。如果提供则根据保证金、杠杆和价格计算数量）
        task_id: 任务ID
        user_id: 用户ID
        api_id: API ID
        trade_trigger_mode: 止盈止损模式（0=关闭, 1=开启）
        tp_trigger_px: 止盈比例（百分比）
        sl_trigger_px: 止损比例（百分比）
        flag: 标志 ("0"=实盘, "1"=模拟盘)
        api_key: API Key
    ...
    计算公式：sums = 保证金 * 杠杆 / 价格
    """
    # 计算交易数量：优先使用直接指定的数量，否则根据保证金计算
    if amount is not None:
        raw_quantity = amount
        print(f"📊 使用直接指定的数量: {amount}")
    elif margin is not None and margin > 0:
        raw_quantity = calculate_quantity_from_margin(margin, leverage, price)
        print(f"📊 根据保证金计算数量: 保证金={margin} USDT, 杠杆={leverage}x, 价格={price}, 数量={raw_quantity:.2f}")
    else:
        raise ValueError("必须提供 amount（数量）或 margin（保证金）之一")
    
    formatted_amount = format_quantity(raw_quantity, symbol, order_type, price)
...
def create_example_1_open_long() -> Dict[str, Any]:
    ...
    return create_trade_json(
        ...
        tp_trigger_px=DEFAULT_TP_TRIGGER_PX,
        sl_trigger_px=DEFAULT_SL_TRIGGER_PX,
        flag="1"
    )
...
def main():
    ...
    if DEFAULT_AMOUNT is not None:
        ...
        calculated_quantity = calculate_quantity_from_margin(DEFAULT_MARGIN, DEFAULT_LEVERAGE, DEFAULT_PRICE)
        print(f"   保证金: {DEFAULT_MARGIN} USDT")
        print(f"   计算数量: {calculated_quantity:.2f} 张 (保证金 * 杠杆 / 价格 = {DEFAULT_MARGIN} * {DEFAULT_LEVERAGE} / {DEFAULT_PRICE})")
