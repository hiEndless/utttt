...
agent_server服务消费event_center生成的最终事件，执行智能分析。它包含两个独立服务：

**主服务（background + final_listen）**：包含两个主要组件：background_main（生成市场背景数据）和final_listen_main（监听final_events并执行信号验证）。
...
**交易决策服务（trade_decision）**：监听l1_events并执行交易决策。此服务独立于主服务运行，不影响background和final_listen服务。
...
- [main.py](file://agent_server/main.py#L1-L21)
- [final_listen_main.py](file://agent_server/final_listen_main.py#L1-L166)
- [trade_decision_main.py](file://agent_server/trade_decision_main.py#L1-L48)
