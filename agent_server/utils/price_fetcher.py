...
if __name__ == "__main__":
    ...
    async def test_get_mark_price():
        # 测试1: indicators 事件（从 Redis 读取）
        event_info_indicators = {
            "route": "indicators",
            "exchange": "binance",
            "symbol": "BTCUSDT",
            "event_id": "BTCUSDT.final.1730000000",
        }
        
        price1 = await get_mark_price(event_info_indicators, "binance")
        print(f"Test 1 (indicators): {price1}")
        
        # 测试2: trade 事件（从 trade_details 读取）
        event_info_trade = {
            "route": "trade",
            "exchange": "binance",
            "symbol": "ETHUSDT",
            "event_id": "binance.ETHUSDT.trade.open.1730000000",
            "meta": json.dumps({
        ...
        price2 = await get_mark_price(event_info_trade, "binance")
        print(f"Test 2 (trade): {price2}")
        
        # 测试3: 直接从 Redis 读取（Hash 结构）
        price3 = await get_mark_price_from_redis("binance", "BTCUSDT", use_hash=True)
        print(f"Test 3 (Redis Hash): {price3}")
    
    asyncio.run(test_get_mark_price())
