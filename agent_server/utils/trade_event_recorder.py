...
class TradeEventRecorder:
    ...
    def _get_trade_ids_from_redis(self, exchange: str,
                                  symbol: str) -> List[str]:
        """
        ...
        try:
            ...
            logger.error(
                f"从 Redis 获取持仓失败: {e}, exchange={exchange}, symbol={symbol}")
            return []
    ...
    def _extract_market_context(self, event_info: Dict[str,
                                                       Any]) -> Dict[str, Any]:
        """
        提取市场背景快照（占位）
    ...
    def _extract_event_data(self, event_info: Dict[str,
                                                   Any]) -> Dict[str, Any]:
        """
        提取事件原始数据
    ...
    def _extract_indicators_snapshot(
            self, event_info: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        提取关键技术指标快照
    ...
    def _get_or_create_trade_id(
            self,
            exchange: str,
            symbol: str,
            position_side: Optional[str] = None) -> List[str]:
        """
        获取当前活跃的 trade_id 列表
        ...
        try:
            with PostgresDB() as db:
                ...
                if results:
                    for row in results:
                        trade_id = row[0] if isinstance(
                            row, tuple) else row.get("trade")
                        if trade_id:
                            trade_ids.append(trade_id)
        ...
        try:
            ...
            with PostgresDB() as db:
                ...
                if existing:
                    ...
                    db_is_verified = existing[1] if isinstance(
                        existing, tuple) else existing.get("is_verified")
                    ...
                    update_params = [
                        ...
                        if indicators_snapshot else None, symbol, exchange,
                        final_is_verified
                    ]
                    ...
                    logger.info(
                        f"事件已更新: trade_id={trade_id}, event_id={event_id}, event_type={event_type}"
                    )
                else:
                    # 不存在，插入新记录
                    sql = """
                    ...
                    params = [
                        ...
                        json.dumps(event_data, ensure_ascii=False),
                        json.dumps(indicators_snapshot, ensure_ascii=False)
                        if indicators_snapshot else None,
                    ...
                    logger.info(
                        f"事件已入库: trade_id={trade_id}, event_id={event_id}, event_type={event_type}"
                    )
    ...
    async def save_event(self,
                         event_info: Dict[str, Any],
                         mark_price: Optional[float] = None) -> bool:
        """
        异步保存事件到数据库
        ...
        try:
            ...
            if not trade_ids:
                logger.debug(
                    f"无法关联 trade_id，跳过入库: exchange={exchange}, symbol={symbol}"
                )
                return False
            ...
            loop = asyncio.get_event_loop()
            save_tasks = [
                loop.run_in_executor(self.executor, self._save_event_sync, tid,
                                     event_info, mark_price)
    ...
    async def update_event_context(self, event_id: str, exchange: str,
                                   symbol: str,
        ...
        try:
            ...
            def _update_sync():
                with PostgresDB() as db:
                    ...
                    logger.info(
                        f"市场背景快照已更新: event_id={event_id}, exchange={exchange}, symbol={symbol}"
                    )
                    return True
            ...
            logger.error(f"更新市场背景快照失败: {e}, event_id={event_id}",
                         exc_info=True)
            return False

    async def save_agent_analysis(self,
                                  event_id: str,
                                  agent_name: str,
        ...
        try:
            ...
            def _save_sync():
                with PostgresDB() as db:
                    ...
                    if not event_rows:
                        logger.warning(
                            f"无法保存分析结果: 未找到对应的事件记录, event_id={event_id}, trade_id={trade_id}"
                        )
                        return False
                    ...
                    final_model_version = model_version or analysis_data.get(
                        "model_version")

                    # 获取实时 mark_price
                    mark_price = get_mark_price_sync(
                        {
                            "symbol": symbol,
                            "exchange": exchange
                        }, exchange)
                    ...
                    for event_db_id in event_db_ids:
                        db.execute(insert_sql, [
                            event_db_id, agent_name, final_model_version,
                            verdict, alignment, suggestion, mark_price,
                            json.dumps(reasoning, ensure_ascii=False)
                            if reasoning else None,
                            json.dumps(full_output,
                                       ensure_ascii=False), symbol, exchange
                        ])

                    logger.info(
                        f"Agent分析结果已保存: {agent_name}, event_id={event_id}, 关联记录数={len(event_db_ids)}"
                    )
                    return True
            ...
            logger.error(f"保存Agent分析结果失败: {e}, agent={agent_name}",
                         exc_info=True)
            return False
