...
from agent_server.agent_workflow.components.result_aggregation import ResultAggregationComponent
from agent_server.agent_workflow.components.persistence import PersistenceComponent
...
class SignalValidationWorkflow(Workflow):
    ...
    def __init__(self, run_id: Optional[str] = None, **kwargs):
        ...
        self.comp_aggregation = ResultAggregationComponent()
        self.comp_persistence = PersistenceComponent(self.run_id)

        super().__init__(
            steps=[
                self.comp_signal_validation.execute,
                self.comp_position_risk.execute,
                self.comp_aggregation.execute,
