...
help_text="$(bash tools/local/bump_event_center_runtime_version.sh --help)"
...
dry_run_out="$(bash tools/local/bump_event_center_runtime_version.sh event-center-runtime-v999 "guard dry run" --apply-from-env-table --dry-run)"
...
if git diff --quiet && git diff --cached --quiet; then
  bash tools/local/bump_event_center_runtime_version.sh event-center-runtime-v999 "guard clean check" --check-clean --dry-run >/dev/null
else
  if bash tools/local/bump_event_center_runtime_version.sh event-center-runtime-v999 "guard clean check" --check-clean --dry-run >/dev/null 2>&1; then
...
if bash tools/local/bump_event_center_runtime_version.sh "$current_version" "guard duplicate log" --no-duplicate-log --dry-run >/dev/null 2>&1; then
  echo "[失败] --no-duplicate-log 未阻止重复版本"
  exit 1
fi

echo "[附加检查] 校验 --strict 组合行为"
if bash tools/local/bump_event_center_runtime_version.sh "$current_version" "guard strict mode" --strict --dry-run >/dev/null 2>&1; then
