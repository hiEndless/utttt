...
RECOMMENDATION_ARTIFACT_STATUS="$(AGENT_SIGNAL_DECISION_REPLAY_RECOMMENDATION_REPORT_PATH="$AGENT_SIGNAL_DECISION_REPLAY_RECOMMENDATION_REPORT_PATH" python3 - <<'PY'
import json
import os
from pathlib import Path

path = str(os.environ.get("AGENT_SIGNAL_DECISION_REPLAY_RECOMMENDATION_REPORT_PATH") or "").strip()
if not path:
    print("missing")
    raise SystemExit(0)
