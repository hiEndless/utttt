...
@router.get(
    "/dashboard/risk/execution/{exchange}/{symbol}/{trade_id}",
    response_model=BaseResponse[Any],
)
async def read_execution_risk(
    exchange: str,
    symbol: str,
    trade_id: str,
    ...
    """
    读取 Redis 仓位风控信息（risk:execution:{exchange}:{symbol}:{trade_id}）
    ...
    key = f"risk:execution:{exchange}:{symbol}:{trade_id}"
    compat_key = f"{key}:"

    try:
        raw = await redis_client.get(key)
        if raw is None:
            raw = await redis_client.get(compat_key)
        return success_response(_try_parse_json(raw))
    except BusinessException:
        raise
    except Exception:
        logger.exception(
            "读取仓位风控信息失败：user_id=%s exchange=%s symbol=%s trade_id=%s key=%s",
            user_id,
            exchange,
            symbol,
            trade_id,
        ...
        raise BusinessException(code=StatusCode.SERVER_ERROR, message="读取仓位风控信息失败")
