...
class KDJBOLLCombo(CompositeComboBase):
    ...
    def build_bullish_triggers(self, ind, prev_ind, kline):
        ...
        return {
            ...
            "low_reversal_mid_break": (j is not None and c.get("j_prev") is not None and c.get("j_prev") < 20 and j >= 20 and mid is not None and close is not None and close > mid),
            ...
            # 下轨支撑（价格接近或触及下轨）
            "near_lower_touch": (lower is not None and close is not None and abs(close - lower) / (close if close else 1) <= self.near_tol),
            ...
            "double_bottom_candidate": (
                ...
                close <= lower * (1 + self.double_tol)
            ),

            # 带宽极窄时的反转提示（作为 trigger，但通常更适合 pattern）
            "band_narrow": (band_width and band_width < self.band_narrow_threshold),

            # 趋势确认上行：K 上穿 50 且收盘在中轨上方
            "trend_confirm_up": (k_prev is not None and k is not None and k_prev < 50 <= k and mid is not None and close is not None and close > mid),
        }
    ...
    def build_bearish_triggers(self, ind, prev_ind, kline):
        ...
        return {
            "high_reversal_mid_break": (j is not None and c.get("j_prev") is not None and c.get("j_prev") > 80 and j <= 80 and mid is not None and close is not None and close < mid),
            ...
            "near_upper_touch": (upper is not None and close is not None and abs(upper - close) / (close if close else 1) <= self.near_tol),
            ...
            "double_top_candidate": (
                ...
                ((mid is None) or (close > mid)) and
                close >= upper * (1 - self.double_tol)
            ),

            "band_narrow": (band_width and band_width < self.band_narrow_threshold),

            "trend_confirm_down": (k_prev is not None and k is not None and k_prev > 50 >= k and mid is not None and close is not None and close < mid),
        }
    ...
    def build_bullish_patterns(self, ind, prev_ind, kline):
        ...
        return {
            ...
            "band_narrow_pattern": (band_width > 0 and band_width < self.band_narrow_threshold),

            # 下轨附近（结构性支撑）
            "near_lower": (lower is not None and close is not None and abs(close - lower) / (close if close else 1) < self.near_tol),
            ...
            "mid_retest": (
                ...
                abs(close - c.get("boll_mid")) / (c.get("boll_mid") if c.get("boll_mid") else 1) < 0.03
            ),

            # double bottom 确认（需要量能放大 or 非窄带）
            "double_bottom_confirm": (c.get("vol_chg") is not None and c.get("vol_chg") > self.vol_ratio_for_confirmation) or (band_width is not None and band_width > self.band_narrow_threshold)
        }
    ...
    def build_bearish_patterns(self, ind, prev_ind, kline):
        ...
        return {
            "band_narrow_pattern": (band_width > 0 and band_width < self.band_narrow_threshold),
            "near_upper": (upper is not None and close is not None and abs(upper - close) / (close if close else 1) < self.near_tol),
            "mid_retest_down": (
                prev_close is not None and close is not None and close < prev_close and
                c.get("boll_mid") is not None and
                abs(close - c.get("boll_mid")) / (c.get("boll_mid") if c.get("boll_mid") else 1) < 0.03
            ),
            "double_top_confirm": (c.get("vol_chg") is not None and c.get("vol_chg") > self.vol_ratio_for_confirmation) or (band_width is not None and band_width > self.band_narrow_threshold)
        }
