...
- 验证脚本清单：`docs/operations/VERIFICATION_SCRIPT_INVENTORY.md`
...
- 契约变更四件套守卫（CI 可用）：`tools/local/check_contract_change_bundle_guard.sh`
...
- 说明：`check_new_arch_guards.sh`（除 `--help` 外）已内置执行 `tools/local/check_docs_contracts_bundle.sh`
