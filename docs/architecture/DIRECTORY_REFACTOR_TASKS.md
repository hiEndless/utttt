...
- [x] 执行 `agent_server_new` 下线 Batch A（阶段1）：删除 `agent_server_new/{runner,pipeline_smoke,memory_summary_runner}.py` 兼容壳并迁移测试引用
...
- [x] 完成第十五个物理迁移试点：`feature_service/providers/{indicators_provider,future_source_providers,__init__}.py` 实现迁入 `services/feature_service/src/providers/`
- [x] 完成第十六个物理迁移试点：`feature_service/normalizers/{response_normalizer,__init__}.py` 实现迁入 `services/feature_service/src/normalizers/`
- [x] 完成第十七个物理迁移试点：`feature_service/ports/*.py` 实现迁入 `services/feature_service/src/ports/`
...
- [x] 执行 `market_state_engine` Batch B（阶段1）：`service.py` 实现迁入 `services/market_state_engine/src/`（旧路径使用模块桥接兼容）
- [x] 执行 `market_state_engine` Batch B（阶段2）：`errors.py` 实现迁入 `services/market_state_engine/src/`（旧路径保留薄兼容壳）
- [x] 执行 `market_state_engine` Batch B（阶段3）：`engine.py` 实现迁入 `services/market_state_engine/src/`（旧路径使用模块桥接兼容）
- [x] 执行 `market_state_engine` Batch B（阶段4）：`msl.py` 实现迁入 `services/market_state_engine/src/`（旧路径保留薄兼容壳）
...
- [x] 收敛 feature/runner/alert 相关守卫脚本到 `tools/local/*`（`scripts/*` 保留薄包装兼容）
- [x] 收敛 event_center contract-schema/runtime 子守卫到 `tools/local/*`（`scripts/*` 保留薄包装兼容）
...
- [x] 解除 `check_new_arch_guards.sh` 的 workflow/text-wiring hard pinning（实逻辑切换至 `tools/local/*`）
