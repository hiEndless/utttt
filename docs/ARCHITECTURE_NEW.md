...
- `execution_service` 在目标架构中是执行层，当前仓库尚未形成独立同名目录。
...
- 职责：消费 `event_center_new + market_state_engine + position_context`，输出 `ExecutionPlan + DecisionTrace`。
- 不负责：原始数据采集、特征计算、状态生成、真实执行。

### 2.6 execution_service（执行层，目标态）
- 职责：执行计划校验、路由下单、成交处理、对账与回执。
