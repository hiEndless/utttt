...
```json
{
  ...
  "schema_mapping_version": "execution-schema-mapping-v24",
  "ts": 1760000000000,
  "ts_ms": 1760000000000
}
```
...
4. `symbol`: 非空字符串
5. `direction_intent`: 规范值 `long | short | neutral`
...
`execution_service` 当前状态提供器仅支持 `redis` 模式：

1. `redis`（默认）：从 Redis 读取仓位/账户/策略状态（Redis 不可用时 fail-open 返回默认状态）
