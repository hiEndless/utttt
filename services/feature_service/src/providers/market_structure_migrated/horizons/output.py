...
if __package__:
    from services.feature_service.src.providers.market_structure_migrated.utils.redis_client import get_verified_redis_client
    ...
    from services.feature_service.src.providers.market_structure_migrated.utils.redis_client import get_verified_redis_client
    from services.feature_service.src.providers.market_structure_migrated.io.background_kline import read_multi_period
    from services.feature_service.src.providers.market_structure_migrated.io.raw_reader import PERIODS, read_market_raw
    from services.feature_service.src.providers.market_structure_migrated.horizons.build_context import (
...
async def build_output(exchange: str, symbol: str) -> Dict[str, Any]:
    ...
    # 统一在同一个事件循环里创建并复用一个 Redis 客户端，避免多套初始化逻辑导致不一致
