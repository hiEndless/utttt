...
运行时配置总表：`services/services/event_center_new/docs/runtime.md`
...
当前已落地：
- `services/services/event_center_new/docs/selected_event.schema.json`
...
`selected_contract` 校验规则直接复用 `services/services/event_center_new/docs/selected_event.schema.json`。
...
`--summary-only` 输出契约冻结为 `services/services/event_center_new/docs/replay_summary.schema.json`。
...
仓库守卫 `tools/local/check_event_center_ci_doc_snapshot_guard.sh` 校验 `services/services/event_center_new/docs/ci.md` 的帮助快照与最短排障命令关键行未漂移（关键行来源：`services/services/event_center_new/docs/ci_help_snapshot_lines.txt` 与 `services/services/event_center_new/docs/ci_triage_snapshot_lines.txt`）。
...
- `services/services/event_center_new/docs/ci.md`（CI 触发矩阵、失败分类、artifact 日志锚点与标准处置）

CI 失败排障请直接查看 `services/services/event_center_new/docs/ci.md`，README 仅保留入口。
