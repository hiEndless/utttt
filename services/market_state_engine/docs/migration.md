...
- 当 `selected_event.context_snapshot.alternative_sources_summary` 可用时，状态层会与 feature 侧来源做融合，产出 `state_features.evidence.alternative_sources_fusion`（有效 feature 优先；若 feature 为 `noop/empty/unavailable` 且无特征则回退 event_center，冲突显式记录）。
