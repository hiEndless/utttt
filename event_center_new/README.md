...
版本升级可使用：`bash scripts/bump_event_center_runtime_version.sh <version> <note>`（支持 `--dry-run` 预览、`--check-clean` 干净工作区保护、`--apply-from-env-table` 环境变量覆盖校验、`--no-duplicate-log` 防重复日志）。当前版本可用 `--print-current-version` 只读查询。
