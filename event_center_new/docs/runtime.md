...
维护建议：新增/删除环境变量时，使用 `scripts/bump_event_center_runtime_version.sh` 同步更新版本与变更日志；可先加 `--dry-run` 预览，或加 `--check-clean` 要求工作区干净。建议同时加 `--apply-from-env-table` 校验 runtime 表格已覆盖 `main.py` 中的 `EVENT_CENTER_*` 变量。
